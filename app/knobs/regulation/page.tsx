...
const typedData = regulationData as unknown as RegulationData;
...
export default function RegulationPage() {
  ...
  const activeScenario =
    typedData.quality_safety_scenarios.find((s) => s.id === activeId) ??
    typedData.market_price_scenarios.find((s) => s.id === activeId) ??
    null;
    ...
    <div>
      ...
      <div className="mt-6 border-2 border-blue-600 bg-white p-6">
        <h2 className="font-blueprint text-sm font-bold text-blue-600 mb-3 uppercase tracking-wider">
          {metric ? metric.name : activeScenario ? activeScenario.name : "Health System Regulation"}
        </h2>
      ...
      <div className="mt-6 border-2 border-rose-600 bg-white p-6">
        <h2 className="font-blueprint text-sm font-bold text-rose-600 mb-4 uppercase tracking-wider">
          Regulation: Quality & Safety
        </h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
          {typedData.quality_safety_scenarios.map((scenario) => (
            <button
              key={scenario.id}
              ...
              className={`text-left px-4 py-3 rounded-sm transition-colors ${
                activeId === scenario.id
                  ...
                  : "border border-slate-300 bg-white hover:border-rose-300"
              }`}
            >
      ...
      <div className="mt-6 border-2 border-rose-600 bg-white p-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
          {typedData.market_price_scenarios.map((scenario) => (
            <button
              key={scenario.id}
              onClick={() => handleScenarioClick(scenario.id)}
              className={`text-left px-4 py-3 rounded-sm transition-colors ${
