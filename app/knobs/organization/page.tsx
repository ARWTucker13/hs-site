...
const typedData = organizationData as unknown as OrganizationData;
...
export default function OrganizationPage() {
  ...
  const activeScenario =
    typedData.delivery_structure_scenarios.find((s) => s.id === activeId) ??
    typedData.facility_governance_scenarios.find((s) => s.id === activeId) ??
    null;
    ...
    <div>
      ...
      <div className="mt-6 border-2 border-blue-600 bg-white p-6">
        <h2 className="font-blueprint text-sm font-bold text-blue-600 mb-3 uppercase tracking-wider">
          {metric ? metric.name : activeScenario ? activeScenario.name : "Health System Organization"}
        </h2>
      ...
      <div className="mt-6 border-2 border-teal-600 bg-white p-6">
        <h2 className="font-blueprint text-sm font-bold text-teal-600 mb-4 uppercase tracking-wider">
          Organization: Delivery Structure
        </h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
          {typedData.delivery_structure_scenarios.map((scenario) => (
            <button
              key={scenario.id}
              ...
              className={`text-left px-4 py-3 rounded-sm transition-colors ${
                activeId === scenario.id
                  ...
                  : "border border-slate-300 bg-white hover:border-teal-300"
              }`}
            >
      ...
      <div className="mt-6 border-2 border-teal-600 bg-white p-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
          {typedData.facility_governance_scenarios.map((scenario) => (
            <button
              key={scenario.id}
              onClick={() => handleScenarioClick(scenario.id)}
              className={`text-left px-4 py-3 rounded-sm transition-colors ${
