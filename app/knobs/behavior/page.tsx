...
const typedData = behaviorData as unknown as BehaviorData;
...
export default function BehaviorPage() {
  ...
  const activeScenario =
    typedData.provider_behavior_scenarios.find((s) => s.id === activeId) ??
    typedData.patient_behavior_scenarios.find((s) => s.id === activeId) ??
    null;
    ...
    <div>
      ...
      <div className="mt-6 border-2 border-blue-600 bg-white p-6">
        <h2 className="font-blueprint text-sm font-bold text-blue-600 mb-3 uppercase tracking-wider">
          {metric ? metric.name : activeScenario ? activeScenario.name : "Persuasion & Behavior Change"}
        </h2>
      ...
      <div className="mt-6 border-2 border-orange-600 bg-white p-6">
        <h2 className="font-blueprint text-sm font-bold text-orange-600 mb-4 uppercase tracking-wider">
          Behavior: Provider Behavior Change
        </h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
          {typedData.provider_behavior_scenarios.map((scenario) => (
            <button
              key={scenario.id}
              ...
              className={`text-left px-4 py-3 rounded-sm transition-colors ${
                activeId === scenario.id
                  ...
                  : "border border-slate-300 bg-white hover:border-orange-300"
              }`}
            >
      ...
      <div className="mt-6 border-2 border-orange-600 bg-white p-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
          {typedData.patient_behavior_scenarios.map((scenario) => (
            <button
              key={scenario.id}
              onClick={() => handleScenarioClick(scenario.id)}
              className={`text-left px-4 py-3 rounded-sm transition-colors ${
